...
export function ConfirmDeleteServiceModal({
  open,
  service,
  isDeleting,
  onClose,
  ...
  if (!open || !service || typeof document === "undefined") {
    return null;
  }
      ...
      <div className="w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-neutral-900" onClick={(event) => event.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-neutral-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-slate-950 dark:text-slate-50">Delete service</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400">This will archive the service from the catalog.</p>
            </div>
          </div>
          <Button type="button" variant="outline" className="h-11 w-11 rounded-xl p-0" onClick={onClose} aria-label="Close delete service modal">
            <X size={24} />
          </Button>
        </div>
        ...
        <div className="space-y-4 p-5">
          <p className="text-sm text-slate-600 dark:text-slate-300">
            Delete <span className="font-semibold text-slate-950 dark:text-slate-50">{service.name}</span> from the catalog?
          </p>
          <div className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-900 dark:border-rose-900/40 dark:bg-rose-950/20 dark:text-rose-100">
            This action hides the service from active selection. It can be restored later from the database.
          </div>
        </div>
