    ...
    <div className="space-y-6">
        ...
        {dentists.map((dentist) => (
          <Card key={dentist.id} className="p-5">
            <div className="flex items-start justify-between gap-4">
              ...
              <Badge className={dentist.availability === "Available" ? "bg-emerald-50 text-emerald-700 ring-emerald-200" : "bg-slate-100 text-slate-700 dark:text-slate-200 ring-slate-200"}>{dentist.availability}</Badge>
            </div>
            <h2 className="mt-5 text-lg font-semibold text-slate-950 dark:text-slate-50">{dentist.name}</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">{dentist.role}</p>
      ...
      {selectedDentist ? (
        <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/30">
          <aside className="h-full w-full max-w-md overflow-y-auto bg-white dark:bg-slate-900 p-5 shadow-2xl">
            <div className="flex items-start justify-between">
              <div>
                <h2 className="text-xl font-semibold text-slate-950 dark:text-slate-50">{selectedDentist.name}</h2>
                <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">{selectedDentist.role} schedule</p>
              </div>
            ...
            <div className="mt-6 space-y-3">
              {selectedDentist.schedule.map((item) => (
                <div key={`${item.time}-${item.patient}`} className="rounded-xl border border-slate-200 dark:border-slate-700 p-4">
                  <p className="font-semibold text-slate-950 dark:text-slate-50">{item.time} - {item.patient}</p>
                  <p className="mt-1 text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">{item.service}</p>
