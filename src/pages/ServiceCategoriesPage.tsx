...
import { FolderTree, Plus, RotateCcw, Search, ShieldCheck } from "lucide-react";
import { ServiceAdminOnlyState } from "../components/services/ServiceAdminOnlyState";
import { ServiceCategoryEditorModal } from "../components/services/ServiceCategoryEditorModal";
...
import { buildCategoryRows, createCategoryColumns } from "../features/services/catalog-display";
...
export function ServiceCategoriesPage() {
  ...
  const rows = useMemo(() => buildCategoryRows(categories, sections, services), [categories, sections, services]);

  const filteredRows = useMemo(() => {
    const query = search.trim().toLowerCase();

    return rows.filter((category) => {
      const statusMatch = statusFilter === "All" || (statusFilter === "Active" ? category.isActive : !category.isActive);
      const queryMatch = !query || [category.name, category.description ?? ""].join(" ").toLowerCase().includes(query);
  ...
  const metrics = useMemo(
    () => ({
      ...
      linkedServices: services.length,
    }),
  ...
  if (!canManage) {
    return <ServiceAdminOnlyState title="Categories" />;
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-sm text-slate-500 dark:text-slate-400">
            Define the top-level service groups used to organize the clinic catalog and reporting structure.
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          { label: "Active categories", value: metrics.activeCategories },
          { label: "Sections configured", value: metrics.totalSections },
          { label: "Services mapped", value: metrics.linkedServices },
        ].map((item) => (
          <Card key={item.label}>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">
                  {filteredRows.length} of {rows.length} categories shown
                </p>
              </div>
            </div>
        ...
        <CardContent className="p-0">
          {error ? (
            ...
            <DataTable
              ...
              emptyDescription={
                hasFilters ? "Try a different search term or clear the filters to see all categories." : "Create a category to organize services by major care area."
              }
              minWidth="760px"
              pagination={{
                page,
          ...
          {isLoading ? <div className="border-t border-slate-100 px-5 py-4 text-sm text-slate-500 dark:border-neutral-800 dark:text-slate-400">Loading categories…</div> : null}
        </CardContent>
      </Card>
