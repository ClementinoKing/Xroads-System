        ...
        {services.map((service) => (
          <Card key={service.id} className="p-5">
            <div className="flex items-start justify-between gap-4">
              ...
              <Badge className={service.active ? "bg-emerald-50 text-emerald-700 ring-emerald-200" : "bg-slate-100 text-slate-700 dark:text-slate-200 ring-slate-200"}>{service.active ? "Active" : "Inactive"}</Badge>
            </div>
            <h2 className="mt-5 text-lg font-semibold text-slate-950 dark:text-slate-50">{service.name}</h2>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">{service.description}</p>
            <div className="mt-5 flex flex-wrap gap-2">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1 text-sm font-semibold text-slate-700 dark:text-slate-200"><Clock size={14} /> {service.duration} min</span>
